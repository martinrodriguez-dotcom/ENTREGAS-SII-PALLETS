...
const App = () => {
  ...
  const initialLoadState = {
    ...
    poNumber: "",
    pallets: "",
    transportName: "",    // Subcategoría flete
    transportDriver: "",  // Subcategoría flete
    transportVehicle: "", // Subcategoría flete
    condition: "",
    paymentCondition: "",
    price: "",
    accountType: "Cta 1",
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      setAuthError(isLoginView ? "Credenciales incorrectas." : "Error: la clave debe tener 6 caracteres.");
    }
  };
  ...
  const requestNotifPermission = async () => {
    if (typeof Notification !== 'undefined') {
      ...
      if (permission === 'granted') {
        new Notification("SII PALLETS", { body: "Alertas activadas", icon: "/logo192.png" });
      }
    }
  };
  ...
  const internalAlerts = useMemo(() => {
    ...
    loads.forEach(load => {
      ...
      // Alerta Flete: Falta al menos el nombre de la empresa
  ...
  const printOC = (oc) => {
    ...
    const content = `<html><head><title>OC ${oc.ocNumber}</title><style>body{font-family:sans-serif;padding:40px;color:#333;}.header{text-align:center;border-bottom:2px solid #065f46;padding-bottom:20px;}.details{margin-top:30px;display:grid;grid-template-cols:1fr 1fr;gap:20px;}.table{width:100%;border-collapse:collapse;margin-top:20px;}.table th,.table td{border:1px solid #ddd;padding:12px;text-align:left;}.table th{background:#f4f4f4;}.footer{margin-top:50px;font-size:11px;text-align:center;color:#94a3b8;} .flete-box{background:#f8fafc;padding:15px;border-radius:10px;margin-top:20px;border:1px solid #e2e8f0;}</style></head><body><div class="header"><h1>ORDEN DE COMPRA INTERNA</h1><p>SII PALLETS LOGÍSTICA</p></div><div class="details"><div><p><strong>N° OC:</strong> #${oc.ocNumber}</p><p><strong>Cliente:</strong> ${oc.customer.toUpperCase()}</p></div><div style="text-align:right;"><p><strong>Fecha:</strong> ${oc.date}</p><p><strong>Turno/Ref:</strong> ${oc.turn || 'N/A'}</p></div></div><div class="flete-box"><p><strong>Transporte:</strong> ${oc.transportName || 'S/D'}</p><p><strong>Chofer:</strong> ${oc.transportDriver || 'S/D'}</p><p><strong>Vehículo:</strong> ${oc.transportVehicle || 'S/D'}</p></div><table class="table"><thead><tr><th>Producto</th><th>Cantidad</th></tr></thead><tbody>${oc.articles.map(a => `<tr><td>${a.name.toUpperCase()}</td><td>${a.qty}</td></tr>`).join('')}</tbody></table><div class="footer">Generado por SII Pallets - ${new Date().toLocaleString()}</div><script>window.onload=function(){window.print();window.close();}</script></body></html>`;
  ...
  // --- VISTAS ---
  if (authLoading) return <div className="h-screen flex items-center justify-center bg-emerald-900 text-white font-black italic animate-pulse">CARGANDO SII PALLETS v4.0...</div>;
    ...
    return (
      <div className="h-screen bg-slate-100 flex items-center justify-center p-6 font-sans">
        <div className="bg-white w-full max-w-sm rounded-[3rem] shadow-2xl p-10 flex flex-col items-center">
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-10 text-center">{isLoginView ? 'Ingreso' : 'Registro'}</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900 pb-24 overflow-x-hidden antialiased">
        ...
        <div className="space-y-4 pb-10">
          ...
          {filteredDayLoads.length > 0 ? filteredDayLoads.map(load => (
            ...
            <div className="text-center py-20 bg-slate-200/10 rounded-[4rem] border-2 border-dashed border-slate-200 flex flex-col items-center">
              ...
              <p className="text-slate-300 font-black uppercase text-[10px] italic tracking-widest">Sin registros</p>
            </div>
          )}
        </div>
      </main>

      {/* --- MODAL: FORMULARIO ENTREGA (ADMIN - EDICION TOTAL) --- */}
      {showForm && isAdmin && (
        <div className="fixed inset-0 bg-slate-900/90 z-[100] flex items-end justify-center backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-t-[3.5rem] shadow-2xl p-8 overflow-y-auto max-h-[92vh] animate-in slide-in-from-bottom-10 duration-500">
            ...
            <form onSubmit={handleSaveLoad} className="space-y-6 pb-6">
              ...
              {/* SECCIÓN FLETE EXPANDIDA */}
              <div className="bg-slate-50 p-6 rounded-[2.5rem] space-y-4 border border-slate-100 shadow-inner">
                <p className="text-[9px] font-black text-slate-400 uppercase flex items-center gap-2 italic"><Truck size={14}/> Datos Logística / Flete</p>
                <input type="text" placeholder="EMPRESA DE TRANSPORTE" value={newLoad.transportName} onChange={e => setNewLoad({...newLoad, transportName: e.target.value})} className="w-full bg-white rounded-xl p-4 text-xs font-black shadow-sm" />
                <div className="grid grid-cols-2 gap-4">
                  <div className="relative">
                    <User size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300"/>
                    <input type="text" placeholder="CHOFER" value={newLoad.transportDriver} onChange={e => setNewLoad({...newLoad, transportDriver: e.target.value})} className="w-full bg-white rounded-xl p-4 pl-9 text-xs font-black shadow-sm" />
                  </div>
                  <div className="relative">
                    <Car size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300"/>
                    <input type="text" placeholder="VEHÍCULO / PATENTE" value={newLoad.transportVehicle} onChange={e => setNewLoad({...newLoad, transportVehicle: e.target.value})} className="w-full bg-white rounded-xl p-4 pl-9 text-xs font-black shadow-sm" />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <input type="number" placeholder="TOTAL PALLETS" required value={newLoad.pallets} onChange={e => setNewLoad({...newLoad, pallets: e.target.value})} className="bg-slate-50 rounded-2xl p-5 text-sm font-black shadow-inner" />
                <div className="flex items-center gap-2 px-2 text-[10px] font-black text-slate-400 uppercase italic">Total Carga</div>
              </div>

              <div className="space-y-4">
                 <div className="flex justify-between items-center px-2"><h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Desglose de Productos</h3><button type="button" onClick={() => setNewLoad(p => ({...p, articles: [...p.articles, {name:"", feature:""}]}))} className="bg-emerald-700 text-white p-2 rounded-xl active:scale-90 shadow-md"><Plus size={16} /></button></div>
      ...
      {/* --- MODAL: NUEVA OC (ADMIN - EDICION TOTAL) --- */}
      {showOCForm && isAdmin && (
        <div className="fixed inset-0 bg-slate-900/90 z-[100] flex items-end justify-center p-0 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-t-[3.5rem] shadow-2xl p-8 overflow-y-auto max-h-[95vh] animate-in slide-in-from-bottom-10 duration-500">
            ...
            <form onSubmit={handleGenerateOC} className="space-y-6 pb-6">
              ...
              {/* SECCIÓN FLETE OC */}
              <div className="bg-slate-50 p-6 rounded-[2.5rem] space-y-4 border border-slate-100 shadow-inner">
                <p className="text-[9px] font-black text-slate-400 uppercase flex items-center gap-2 italic"><Truck size={14}/> Logística / Flete Sugerido</p>
                <input type="text" placeholder="EMPRESA TRANSPORTE" value={newOC.transportName} onChange={e => setNewOC({...newOC, transportName: e.target.value})} className="w-full bg-white rounded-xl p-4 text-xs font-black shadow-sm" />
                <div className="grid grid-cols-2 gap-4">
                  <input type="text" placeholder="CHOFER" value={newOC.transportDriver} onChange={e => setNewOC({...newOC, transportDriver: e.target.value})} className="bg-white rounded-xl p-4 text-xs font-black shadow-sm" />
                  <input type="text" placeholder="VEHÍCULO" value={newOC.transportVehicle} onChange={e => setNewOC({...newOC, transportVehicle: e.target.value})} className="bg-white rounded-xl p-4 text-xs font-black shadow-sm" />
                </div>
              </div>
              ...
              <div className="space-y-4">
                 <div className="flex justify-between items-center px-2"><h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Artículos Detallados</h3><button type="button" onClick={() => setNewOC({...newOC, articles: [...newOC.articles, {name:"", qty:""}]})} className="bg-indigo-600 text-white p-2 rounded-xl active:scale-90 shadow-md transition-all"><Plus size={18} /></button></div>
            ...
            <div className="p-8 space-y-6 overflow-y-auto max-h-[60vh] hide-scrollbar bg-white">
              {isAdmin && (
                <div className="bg-emerald-50 p-6 rounded-[2.5rem] border-2 border-emerald-100 space-y-3 animate-in fade-in shadow-sm">
                  <p className="text-[10px] font-black text-emerald-800 uppercase flex items-center gap-2 italic"><DollarSign size={12}/> Finanzas</p>
                  <div className="flex justify-between items-center text-sm font-black"><span className="text-emerald-900 text-xl font-black italic">$ {viewLoad.price || '0'}</span><span className="bg-emerald-800 text-white px-4 py-1.5 rounded-xl text-[10px] shadow-sm">{viewLoad.accountType}</span></div>
        ...
        <div className="fixed inset-0 bg-slate-900/90 z-[160] flex items-center justify-center p-6 backdrop-blur-md animate-in fade-in transition-all">
          <div className="bg-white w-full max-w-md rounded-[3.5rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95">
            <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-emerald-50">
              ...
              <button onClick={() => setShowAlerts(false)} className="p-4 bg-white rounded-2xl active:scale-90 transition-all shadow-sm"><X size={20} /></button>
            </div>
            <div className="p-8 overflow-y-auto max-h-[60vh] hide-scrollbar text-center">
              <button onClick={requestNotifPermission} className="w-full py-4 bg-emerald-800 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest mb-6 active:scale-95 transition-all shadow-lg flex items-center justify-center gap-3">
                {notifPermission === 'granted' ? <><Check size={16}/> Alertas Activas</> : <><BellRing size={16}/> Activar Notificaciones</>}
              </button>
              {internalAlerts.length > 0 ? internalAlerts.map((a, i) => (
                <div key={`alert-i-${i}`} onClick={() => { 
                  ...
                  <div>
                    ...
                    <p className="text-[8px] font-black text-slate-400 mt-1 italic uppercase tracking-widest">Toca para abrir →</p>
                  </div>
                </div>
              )) : (
        ...
        <div className="fixed inset-0 bg-slate-900/90 z-[200] flex items-center justify-center p-6 backdrop-blur-md animate-in fade-in">
          <div className="bg-white w-full max-w-lg rounded-[3.5rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95">
            <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-indigo-50">
              <div><h2 className="text-xl font-black text-indigo-900 uppercase italic">Historial OCs</h2><p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mt-1 italic">Sincronización Total</p></div>
              <button onClick={() => setShowOCHistory(false)} className="p-3 bg-white rounded-2xl active:scale-90 transition-all shadow-sm"><X size={20} /></button>
            </div>
            <div className="p-4 bg-slate-50 border-b relative">
              ...
              <input type="text" placeholder="BUSCAR POR CLIENTE O N°..." value={ocSearchQuery} onChange={e => setOcSearchQuery(e.target.value)} className="w-full bg-white rounded-xl py-3 pl-12 pr-6 text-[10px] font-black uppercase shadow-inner" />
            </div>
            <div className="p-4 space-y-3 max-h-[60vh] overflow-y-auto hide-scrollbar bg-white">
              {filteredOCHistory.map((oc) => (
                <div key={`h-oc-${oc.id}`} className="bg-white border-2 border-slate-50 p-4 rounded-[2rem] flex items-center justify-between shadow-sm hover:border-indigo-200 transition-all">
                   <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className={`text-[8px] font-black px-2 py-0.5 rounded-full uppercase ${oc.isUsed ? 'bg-slate-100 text-slate-400' : 'bg-emerald-100 text-emerald-700'}`}>{oc.isUsed ? 'PROCESADA' : 'DISPONIBLE'}</span>
                        <p className="text-[10px] font-black text-indigo-600 uppercase tracking-tighter">N° {oc.ocNumber}</p>
                      </div>
                      <p className="text-xs font-black text-slate-900 uppercase mt-1 truncate max-w-[150px]">{oc.customer}</p>
                      <p className="text-[9px] font-bold text-slate-400">{oc.date}</p>
                   </div>
                   <button onClick={() => printOC(oc)} className="p-4 bg-slate-50 rounded-2xl text-slate-500 hover:bg-indigo-600 hover:text-white transition-all active:scale-90 shadow-sm"><Printer size={20}/></button>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/80 z-[250] flex items-center justify-center p-6 backdrop-blur-md animate-in fade-in transition-all">
          <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95">
            ...
            <div className="p-4 space-y-3 max-h-[60vh] overflow-y-auto hide-scrollbar">
              ...
              )) : <div className="text-center py-10"><p className="text-xs font-black text-slate-300 uppercase tracking-widest italic">Sin OCs pendientes</p></div>}
            </div>
          </div>
        </div>
      )}
      ...
      {shareLoad && (
        <div className="fixed inset-0 bg-slate-900/90 z-[210] flex items-center justify-center p-6 backdrop-blur-md animate-in fade-in transition-all">
          <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl p-10 flex flex-col overflow-hidden animate-in zoom-in-95">
            ...
            <button onClick={() => copyToClipboard(formatWhatsAppMessage(shareLoad))} className={`w-full py-7 rounded-[2.5rem] font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-4 transition-all shadow-xl ${copyFeedback ? 'bg-emerald-100 text-emerald-800 border-2 border-emerald-200' : 'bg-emerald-800 text-white active:scale-95'}`}>{copyFeedback ? '¡MENSAJE COPIADO!' : 'COPIAR PARA WHATSAPP'}</button>
          </div>
        </div>
      )}
      ...
      {quickStatusLoad && isAdmin && (
        <div className="fixed inset-0 bg-slate-900/60 z-[150] flex items-center justify-center p-12 backdrop-blur-sm animate-in fade-in transition-all">
          <div className="bg-white w-full max-w-xs rounded-[3rem] shadow-2xl p-8 animate-in zoom-in-95">
            <h3 className="text-center font-black text-slate-400 text-[10px] uppercase mb-8 tracking-widest italic">Cambiar Proceso</h3>
            <div className="space-y-4">
