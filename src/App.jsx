...
import { 
  ...
  Share2, Copy, Check, Bell, BellRing, FilePlus, FileText, Printer, ArrowRight, AlertCircle, AlertTriangle, Eye, ListFilter, ClipboardList, Lock, LogOut, DollarSign, UserPlus, LogIn, CreditCard
} from 'lucide-react';
...
const App = () => {
  ...
  // --- LOGICA DE AUTH Y ROLES ---
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        // Ruta obligatoria para perfiles privados
        ...
        if (docSnap.exists()) {
          ...
          // Creación automática de perfil para nuevos usuarios
          await setDoc(docRef, { 
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      if (err.code === 'auth/weak-password') {
        ...
        setAuthError(isLoginView ? "Credenciales incorrectas." : "Error al procesar la solicitud.");
      }
    }
  };
  ...
  // --- SINCRONIZACIÓN DATOS (FIRESTORE) ---
  useEffect(() => {
    if (!user) return;
  ...
  // Autocompletado inteligente desde OC
  const selectOC = (oc) => {
    const totalQtySuma = oc.articles.reduce((sum, art) => sum + (Number(art.qty) || 0), 0);
    setNewLoad({
      ...initialLoadState,
      ...
      pallets: totalQtySuma > 0 ? totalQtySuma.toString() : "",
  ...
  // --- VISTA ACCESO ---
  if (authLoading) return <div className="h-screen flex items-center justify-center bg-emerald-900 text-white font-black italic animate-pulse">SII PALLETS v3.2...</div>;

  if (!user) {
    return (
      <div className="h-screen bg-slate-100 flex items-center justify-center p-6 font-sans">
        <div className="bg-white w-full max-w-sm rounded-[3rem] shadow-2xl p-10 flex flex-col items-center animate-in fade-in zoom-in-95">
          ...
          <form onSubmit={handleAuth} className="w-full space-y-4">
             <input type="email" placeholder="EMAIL DE EMPRESA" required value={email} onChange={e => setEmail(e.target.value)}
              className="w-full bg-slate-50 border-none rounded-2xl p-5 text-xs font-black uppercase shadow-inner outline-none focus:ring-4 focus:ring-emerald-500/10 transition-all" />
             <input type="password" placeholder="CONTRASEÑA" required value={password} onChange={e => setPassword(e.target.value)}
              className="w-full bg-slate-50 border-none rounded-2xl p-5 text-xs font-black uppercase shadow-inner outline-none focus:ring-4 focus:ring-emerald-500/10 transition-all" />
             
             {authError && <p className="text-[9px] font-black text-rose-500 text-center uppercase leading-relaxed">{authError}</p>}
             
             <button type="submit" className="w-full bg-emerald-800 text-white py-5 rounded-[2rem] font-black uppercase text-sm shadow-xl active:scale-95 transition-all">
                {isLoginView ? 'INGRESAR' : 'REGISTRARME'}
             </button>
          </form>

          <button onClick={() => setIsLoginView(!isLoginView)} className="mt-6 text-[9px] font-black text-emerald-600 uppercase tracking-widest hover:underline">
            {isLoginView ? '¿Nuevo aquí? Crea una cuenta' : '¿Ya tienes cuenta? Inicia sesión'}
          </button>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900 pb-24 overflow-x-hidden antialiased">
      ...
      <header className="bg-emerald-800 text-white p-6 rounded-b-[2.5rem] shadow-xl relative flex-shrink-0">
        <div className="flex justify-between items-start relative z-10">
          <div>
            <h1 className="text-2xl font-black uppercase italic">SII PALLETS</h1>
            <div className="flex items-center gap-2 mt-2">
              <span className="text-[9px] font-bold bg-emerald-900/40 px-3 py-1 rounded-full border border-emerald-500/20 uppercase">
                {userRole === 'admin' ? 'ADMINISTRADOR' : 'LOGÍSTICA'}
              </span>
              <button onClick={handleLogout} className="p-1 text-emerald-300 hover:text-white transition-colors"><LogOut size={16}/></button>
            </div>
          </div>
          <button onClick={() => setShowAlerts(true)} className="p-3 bg-white/10 rounded-2xl relative active:scale-90 transition-all">
            <Bell size={20} />
            {internalAlerts.length > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 bg-rose-500 text-[10px] font-black rounded-full flex items-center justify-center border-2 border-emerald-800 animate-bounce">{internalAlerts.length}</span>}
          </button>
        </div>

        <div className="mt-6 flex justify-between items-center bg-white/10 p-2 rounded-2xl border border-white/5">
          <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, 1))} className="p-2"><ChevronLeft size={20} /></button>
          <h2 className="text-xs font-black uppercase tracking-widest">{monthNames[currentDate.getMonth()]} {currentDate.getFullYear()}</h2>
          <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 1))} className="p-2"><ChevronRight size={20} /></button>
        </div>
      </header>

      <main className="px-5 -mt-6 relative z-20">
        {/* CALENDARIO */}
        <div className="bg-white rounded-[2.5rem] shadow-xl p-6 border border-slate-100 mb-8 animate-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="space-y-4">
          <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-4 flex items-center gap-2 mb-2 italic"><ClipboardList size={14}/> Hoja de Ruta</h3>
                ...
                <div className="flex justify-between items-start mb-4">
                   ...
                   {isAdmin && (
                    <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                        <button onClick={() => { setEditingId(load.id); setNewLoad(load); setShowForm(true); }} className="p-2 text-slate-200 hover:text-blue-500 transition-colors"><Edit3 size={18} /></button>
                        <button onClick={() => deleteLoad(load.id)} className="p-2 text-slate-200 hover:text-rose-500 transition-colors"><Trash2 size={18} /></button>
                    </div>
                   )}
                </div>
                ...
                {load.articles && load.articles.length > 0 && load.articles[0].name && (
                  <div className="flex flex-wrap gap-2 border-t border-slate-50 pt-3">
                    {load.articles.map((art, i) => (
                      <div key={`${load.id}-art-${i}`} className="bg-slate-100/50 px-3 py-1.5 rounded-xl border border-slate-200/20">
                         <span className="text-[9px] font-black uppercase text-slate-600">{art.name}</span>
                         {art.feature && <span className="text-[8px] font-bold text-slate-400 ml-1 italic">({art.feature})</span>}
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div className="fixed inset-0 bg-slate-900/90 z-[100] flex items-end justify-center p-0 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-t-[3.5rem] shadow-2xl p-8 overflow-y-auto max-h-[95vh] animate-in slide-in-from-bottom-10 duration-500">
            ...
            <form onSubmit={handleGenerateOC} className="space-y-6">
              <input type="text" required placeholder="CLIENTE" value={newOC.customer} onChange={e => setNewOC({...newOC, customer: e.target.value})} className="w-full bg-slate-50 border-none rounded-2xl p-5 text-sm font-black uppercase shadow-inner" />
              
              <div className="bg-emerald-50 p-6 rounded-[2.5rem] space-y-4 border border-emerald-100">
                <p className="text-[9px] font-black text-emerald-800 uppercase flex items-center gap-2 italic"><DollarSign size={14}/> Administración / Finanzas</p>
              ...
              <div className="space-y-4">
                 <div className="flex justify-between items-center px-2">
                   ...
                   <button type="button" onClick={() => setNewOC({...newOC, articles: [...newOC.articles, {name:"", qty:""}]})} className="bg-indigo-600 text-white p-2 rounded-xl active:scale-90 transition-all shadow-md"><Plus size={18} /></button>
                 </div>
      ...
      {showForm && isAdmin && (
        <div className="fixed inset-0 bg-slate-900/90 z-[100] flex items-end justify-center backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-t-[3.5rem] shadow-2xl p-8 overflow-y-auto max-h-[92%] animate-in slide-in-from-bottom-10 duration-500">
            <div className="flex justify-between items-center mb-8 border-b border-slate-100 pb-4">
              <h2 className="text-xl font-black text-slate-800 uppercase italic">Registro de Entrega</h2>
              <button onClick={() => { setShowForm(false); setSelectedInternalOCId(null); }} className="p-3 bg-slate-100 rounded-2xl active:scale-90 transition-all"><X size={20} /></button>
            </div>
            <form onSubmit={handleSaveLoad} className="space-y-6">
              <div className="flex gap-2">
                {['Pendiente', 'En Proceso', 'Entregado'].map(s => (
                  <button key={`st-opt-${s}`} type="button" onClick={() => setNewLoad({...newLoad, status: s})} 
                    className={`flex-1 py-4 rounded-2xl text-[10px] font-black uppercase border-2 transition-all 
                    ${newLoad.status === s ? 'bg-emerald-800 border-emerald-800 text-white shadow-xl shadow-emerald-100' : 'bg-white text-slate-300 border-slate-100'}`}>{s}</button>
                ))}
              </div>
              ...
              <div className="bg-emerald-50 p-6 rounded-[2.5rem] space-y-4 border border-emerald-100">
                <p className="text-[10px] font-black text-emerald-800 uppercase flex items-center gap-2 italic"><DollarSign size={14}/> Finanzas</p>
                <div className="grid grid-cols-2 gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <input type="text" placeholder="FLETE" value={newLoad.transport} onChange={e => setNewLoad({...newLoad, transport: e.target.value})} className="bg-slate-50 rounded-2xl p-5 text-sm font-black shadow-inner" />
              </div>
          ...
          <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95">
            ...
            <div className="p-4 space-y-3 max-h-[60vh] overflow-y-auto hide-scrollbar">
              {availableOCs.length > 0 ? availableOCs.map((oc) => (
                <div key={`picker-oc-item-${oc.id}`} onClick={() => selectOC(oc)}
                ...
                <div className="text-center py-10">
                   <p className="text-xs font-black text-slate-300 uppercase tracking-widest italic">No hay OCs para procesar</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white w-full max-w-md rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
            <div className="p-8 bg-emerald-800 text-white flex justify-between items-start">
              <div>
                ...
                <p className="text-emerald-200 text-[10px] font-bold mt-2 uppercase tracking-widest">{viewLoad.date}</p>
              </div>
            ...
            <div className="p-8 space-y-6 overflow-y-auto max-h-[60vh] hide-scrollbar bg-white">
              ...
              {isAdmin && (
                <div className="bg-emerald-50 p-6 rounded-[2.5rem] border-2 border-emerald-100 space-y-3 animate-in fade-in zoom-in-95">
                  <p className="text-[10px] font-black text-emerald-800 uppercase tracking-widest flex items-center gap-2 italic"><DollarSign size={12}/> Datos Financieros</p>
                  <div className="flex justify-between items-center text-sm font-black">
                     <span className="text-emerald-900 text-xl font-black">$ {viewLoad.price || '0'}</span>
                     <span className="bg-emerald-800 text-white px-4 py-1.5 rounded-xl text-[10px] shadow-sm">{viewLoad.accountType}</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs font-bold text-emerald-600 uppercase border-t border-emerald-100 pt-3">
                    <CreditCard size={14}/>
                    <span>{viewLoad.paymentCondition || 'Condición no especificada'}</span>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 text-center shadow-inner">
                   <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-1">Orden Compra</p>
              ...
              <div className="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 shadow-inner">
                <p className="text-[9px] font-black text-slate-400 uppercase mb-2 flex items-center gap-2 italic tracking-widest"><Truck size={14}/> Datos Logística</p>
                <p className="text-sm font-black text-slate-900">{viewLoad.transport || 'Flete Pendiente'}</p>
              ...
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase mb-3 tracking-widest italic flex items-center gap-2"><ListFilter size={12}/> Productos</p>
                <div className="space-y-2">
                   {viewLoad.articles?.map((art, i) => (
                      <div key={`va-it-${i}`} className="bg-white border border-slate-100 p-4 rounded-2xl shadow-sm flex justify-between items-center hover:bg-slate-50 transition-all">
          ...
          <div className="bg-white w-full max-w-md rounded-[3.5rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95">
            ...
            <div className="p-8 overflow-y-auto max-h-[60vh] hide-scrollbar text-center">
              {internalAlerts.length > 0 ? internalAlerts.map((a, i) => (
                <div key={`alert-i-${i}`} className="bg-rose-50 p-5 rounded-3xl border-2 border-rose-100 flex items-center gap-4 mb-3 text-left">
                  <AlertTriangle className="text-rose-600 shrink-0" />
                  <div>
                    <p className="text-[10px] font-black text-rose-900 uppercase">{a.title}</p>
                    <p className="text-xs font-bold text-rose-500">{a.message}</p>
                  </div>
                </div>
              )) : (
                <div className="py-10">
